...
import { insertRequestSchema, updateRequestSchema } from "@shared/schema";
    ...
    try {
      ...
      if (request.tier === 'basic') {
        const basicResponses: Record<string, string> = {
          'Marketing Strategy': 'For effective Marketing Strategy: Define your unique selling proposition (USP). Focus on 2-3 key channels where your audience hangs out. Consistent content creation is key to building trust.',
          'Operational Efficiency': 'To improve Operational Efficiency: Document all standard operating procedures (SOPs). Identify bottlenecks by mapping out your value stream. Automate repetitive tasks where possible.',
          'Financial Planning': 'For robust Financial Planning: Maintain a 6-month cash runway. Review P&L statements monthly. Separate personal and business finances completely.',
          'HR & Hiring': 'HR Best Practice: Hire for culture fit as much as skill. Implement a structured onboarding process (30-60-90 day plan). Regular feedback loops prevent surprise turnovers.',
          'default': 'Thank you for your inquiry. Our standard advice is to focus on clear goal setting, metric tracking, and consistent execution.'
        };
        
        const topic = request.description.replace('Selected Topic: ', '');
        const response = basicResponses[topic] || basicResponses['default'];
